...
public class Exercise1 {
    ...
    public static void main(String[] args) {
        ...
        StreamSources.bringUsers().forEach(user -> {
            String[] splittedName = user.getName().split(" ");
            out.println(splittedName[0]);
        });

        // TODO: Print first names in userStream for users that have IDs from number stream
        StreamSources.bringUsers()
                // Filter users which ID is not contained in Stream of IDS
                .filter(
